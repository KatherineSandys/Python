...
def main():
    ...
    ind = MSE.index(min(MSE)) #min index
...
def normalize_train(X_train):
    ...
    #for i in range(9): #9x9 matrix
    #    for j in range(9):
    #        X[i][j] = (X_train[i][j] - mean[i]) / std[i]
...
def normalize_test(X_test, trn_mean, trn_std):
    ...
    #X[8][8] = 0 #sets the return matrix up

    #for i in range(9): #9x9 matrix
     #   for j in range(9):
    #        X[i][j] = (X_test - trn_mean) / trn_std
